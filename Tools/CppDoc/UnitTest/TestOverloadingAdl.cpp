...
TEST_FILE
{
	TEST_CATEGORY(L"Argument-dependent lookup")
	{
	...
	TEST_CATEGORY(L"Argument-dependent lookup: Re-indexing functions")
	{
		auto input = LR"(
namespace x
{
...
int f = F(x::X());
	...
	TEST_CATEGORY(L"Argument-dependent lookup: Re-indexing operators 1")
	{
		auto input = LR"(
namespace x
{
...
int c = x::X()+1;
		...
		COMPILE_PROGRAM_WITH_RECORDER(program, pa, input, recorder);
		TEST_CASE_ASSERT(accessed.Count() == 15);
	});

	TEST_CATEGORY(L"Argument-dependent lookup: Re-indexing operators 2")
	{
		auto input = LR"(
struct X
{
