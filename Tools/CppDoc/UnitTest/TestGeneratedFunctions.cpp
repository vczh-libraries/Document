...
struct TestGC_Helper
{
	...
	{
		auto classSymbol = pa.context->TryGetChildren_NFb(L"test_generated_functions")->Get(0)->TryGetChildren_NFb(name)->Get(0).Obj();
...
struct B
{
	Unfeatured unfeatured;
		...
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"A")->Get(0).Obj(), SpecialMemberKind::DefaultCtor) == false);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"B")->Get(0).Obj(), SpecialMemberKind::DefaultCtor) == false);
	}
	{
		auto input = LR"(
struct Unfeatured
...
struct B
{
	Unfeatured unfeatured;
		...
		COMPILE_PROGRAM(program, pa, input);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"A")->Get(0).Obj(), SpecialMemberKind::CopyCtor) == false);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"B")->Get(0).Obj(), SpecialMemberKind::CopyCtor) == false);
	}
	{
		auto input = LR"(
struct Unfeatured
...
struct B
{
	Unfeatured unfeatured;
		...
		COMPILE_PROGRAM(program, pa, input);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"A")->Get(0).Obj(), SpecialMemberKind::MoveCtor) == false);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"B")->Get(0).Obj(), SpecialMemberKind::MoveCtor) == false);
	}
	{
		auto input = LR"(
struct Unfeatured
...
struct B
{
	Unfeatured unfeatured;
		...
		COMPILE_PROGRAM(program, pa, input);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"A")->Get(0).Obj(), SpecialMemberKind::CopyAssignOp) == false);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"B")->Get(0).Obj(), SpecialMemberKind::CopyAssignOp) == false);
	}
	{
		auto input = LR"(
struct Unfeatured
...
struct B
{
	Unfeatured unfeatured;
		...
		COMPILE_PROGRAM(program, pa, input);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"A")->Get(0).Obj(), SpecialMemberKind::MoveAssignOp) == false);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"B")->Get(0).Obj(), SpecialMemberKind::MoveAssignOp) == false);
	}
	{
		auto input = LR"(
struct Unfeatured
...
struct B
{
	Unfeatured unfeatured;
		...
		COMPILE_PROGRAM(program, pa, input);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"A")->Get(0).Obj(), SpecialMemberKind::Dtor) == false);
		TEST_ASSERT(IsSpecialMemberFeatureEnabled(pa, pa.context->TryGetChildren_NFb(L"B")->Get(0).Obj(), SpecialMemberKind::Dtor) == false);
	}
}