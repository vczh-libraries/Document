...
- [ ] Parse full expressions
  - [ ] Add a policy description struct to `Parse*Type` and `Parse*Declarator` so that sometimes when a name fail to resolve it could also be considered part of a type
    - `ParseIdType` succeeds even if unable to resolve
    - `ParseChildType`
    ...
    - `ParseLongType` passes switch to `ParseChildType`
      - `ParseDeclaratorName` passes switch to `ParseLongType`, this is for `operator TYPE` special name
        - `operator TYPE` needs a type and itself is not a type name, so there is not structure like `operator TYPE::NAME`. Here the switch is `SUCCESS`
      - `ParseTypeBeforeDeclarator` passes switch to `ParseLongType`, this is for creating `MemberType`
        - Here the switch is `FAIL-SYNTAX`
        - `ParseSingleDeclarator` adds tokens: `CALLING-CONVENTION`, `LPARENTHESIS`, `LBRACKET`
          - `ParseSingleDeclarator` adds tokens: `RPARENTHESIS`
          - `ParseDeclaratorWithInitializer` add tokens: `EQ`, `LPARENTHESIS`, `LBRACE`, `COMMA`, this is for initializers
            - `ParseDeclarator` add tokens: from parameter
      - `ParseDeclarator` passes switch to `ParseLongType`, this is for non-special declarator
        - `ParseMemberDeclarator` add tokens: from parameter
          - `ParseDeclaration` add tokens: `LBRACE`, `SEMICOLON`
        - `ParseNonMemberDeclarator` Here the switch is `SUCCESS`
        - `ParseType` passes switch to `ParseDeclarator`. Here the switch is `SUCCESS` or `FAIL-UNRESOLVE`, according to parameter
  - [ ] Assert ASTs
